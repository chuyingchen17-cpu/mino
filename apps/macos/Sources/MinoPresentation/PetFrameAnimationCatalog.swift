...
public struct PetFrameAnimation {
    ...
    public var clipDuration: TimeInterval {
        frameDuration * Double(max(textures.count, 1))
    }

    /// Plays at the authored frame rate. Reaction duration may loop a one-shot
    /// or hold the last frame, but it never stretches a 4-frame clip into a crawl.
    public func playbackAction(fitting duration: TimeInterval) -> SKAction {
        ...
        if loops {
            return .repeatForever(animate)
        }
        let playDuration = clipDuration
        if duration <= playDuration + 0.001 {
            return animate
        }
        let extra = duration - playDuration
        let extraLoops = Int(extra / playDuration)
        let hold = extra - Double(extraLoops) * playDuration
        var actions: [SKAction] = [animate]
        if extraLoops > 0 {
            actions.append(.repeat(animate, count: extraLoops))
        }
        if hold > 0.001 {
            actions.append(.wait(forDuration: hold))
        }
        return actions.count == 1 ? animate : .sequence(actions)
    }
...
public final class PetFrameAnimationCatalog {
    ...
    private func loadAnimation(
        ...
        for frameURL in frameURLs {
            guard
                ...
                canvasPixelScale(bitmap, canvas: manifest.canvas) != nil,
                bitmap.hasAlpha,
                hasTransparentCanvasEdges(bitmap)
            else {
                validationFailures.append(
                    "\(frameURL.lastPathComponent) must be a transparent 120x120 or 240x240 PNG whose content stays inside the canvas"
                )
                return nil
            }
            let image = NSImage(
    ...
    private func canvasPixelScale(
        _ bitmap: NSBitmapImageRep,
        canvas: PetFrameCanvasManifest
    ) -> Int? {
        guard canvas.width > 0, canvas.height > 0 else { return nil }
        let scale = bitmap.pixelsWide / canvas.width
        guard scale == 1 || scale == 2 else { return nil }
        guard bitmap.pixelsWide == canvas.width * scale,
              bitmap.pixelsHigh == canvas.height * scale else {
            return nil
        }
        return scale
    }
