...
private struct ServerDecisionRequest: Encodable, Sendable {
    ...
    struct State: Encodable, Sendable {
        ...
        let ownerPresence: OwnerPresence
        let ownerActivity: String
        let companionPresent: Bool
        let careFullness: PetCareBand?
        let careEnergy: PetCareBand?
        let careMood: PetCareBand?
    }
    ...
    init(_ request: ManagedModelRequest) {
        ...
        state = State(
            ...
            ownerPresence: context.state.ownerPresence,
            ownerActivity: context.state.ownerActivity.wireValue,
            companionPresent: context.state.companionPresent,
            careFullness: context.state.publicCare?.fullness,
            careEnergy: context.state.publicCare?.energy,
            careMood: context.state.publicCare?.mood
        )
