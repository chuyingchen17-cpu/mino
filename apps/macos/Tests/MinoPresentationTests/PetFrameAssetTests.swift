...
func idleFramesKeepACompleteBodyAndSeparatedFeetAtTheSharedAnchor() throws {
    for character in PetCharacterID.allCases {
        for frame in try PetFrameSourceTree.frames(character: character, clip: .idle) {
            ...
            let scale = max(1, bitmap.pixelsWide / 120)
            #expect(
                bounds.height >= 60 * scale && Double(bounds.height) / Double(bounds.width) >= 0.85,
                "Idle silhouette must include the torso and legs, not only a head: \(frame.url.path)"
            )
            #expect(
                (99 * scale...102 * scale).contains(bounds.maxY),
                "Idle feet must finish at the shared y=102 ground band: \(frame.url.path)"
            )

            let footRows = max(bounds.minY, bounds.maxY - 5 * scale)...bounds.maxY
            let hasSeparatedFeet = footRows.contains { y in
                let leftFoot = (bounds.minX..<(56 * scale)).contains {
                    (bitmap.colorAt(x: $0, y: y)?.alphaComponent ?? 0) > 0.10
                }
                let rightFoot = ((65 * scale)...bounds.maxX).contains {
                    (bitmap.colorAt(x: $0, y: y)?.alphaComponent ?? 0) > 0.10
                }
                let centerGap = ((56 * scale)...(64 * scale)).allSatisfy {
                    (bitmap.colorAt(x: $0, y: y)?.alphaComponent ?? 0) <= 0.10
                }
    ...
    for character in PetCharacterID.allCases {
        for clip in PetMotionClipID.allCases {
            for frame in try PetFrameSourceTree.frames(character: character, clip: clip) {
                ...
                let scale = bitmap.pixelsWide / 120
                #expect(
                    scale == 1 || scale == 2,
                    "Frame must be 120 or 240 pixels wide: \(frame.url.path)"
                )
                #expect(bitmap.pixelsWide == 120 * scale, "Wrong width: \(frame.url.path)")
                #expect(bitmap.pixelsHigh == 120 * scale, "Wrong height: \(frame.url.path)")
                #expect(bitmap.hasAlpha, "Frame must preserve an alpha channel: \(frame.url.path)")

                let last = bitmap.pixelsWide - 1
                let edgePixels = (0..<bitmap.pixelsWide).flatMap { coordinate in
                    [
                        bitmap.colorAt(x: coordinate, y: 0),
                        bitmap.colorAt(x: coordinate, y: last),
                        bitmap.colorAt(x: 0, y: coordinate),
                        bitmap.colorAt(x: last, y: coordinate)
                    ]
                }
                #expect(
                ...
                let groundAnchorY = 102 * scale
                let belowGroundPixels = ((groundAnchorY + 1)..<bitmap.pixelsHigh).flatMap { y in
                    ((24 * scale)..<(96 * scale)).map { x in bitmap.colorAt(x: x, y: y) }
                }
                #expect(
                ...
                let footContactBand = ((groundAnchorY - 4 * scale)...groundAnchorY).flatMap { y in
                    ((24 * scale)..<(96 * scale)).map { x in bitmap.colorAt(x: x, y: y) }
                }
                #expect(
...
func packagedFrameCatalogCoversEveryClipAndUsesOneCanvasAndGroundAnchor() throws {
    ...
    for character in PetCharacterID.allCases {
        ...
        for clip in PetMotionClipID.allCases {
            ...
            #expect(animation.textures.allSatisfy { $0.filteringMode == .linear })
        }
    }
}
...
func defaultSpriteKitRendererUsesRasterFramesAndNeverBuildsVectorBodyShapes() throws {
    ...
    for character in PetCharacterID.allCases {
        for clip in PetMotionClipID.allCases {
            ...
            #expect(sprites.allSatisfy { $0.texture?.filteringMode == .linear })
