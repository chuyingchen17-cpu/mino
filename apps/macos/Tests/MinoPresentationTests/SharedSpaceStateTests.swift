...
@MainActor
@Test func sharedSpaceModelDerivesSelectedFriendAndTimelineOrder() {
    let selected = FriendshipID(rawValue: "00000000-0000-4000-8000-000000000002")
    let older = PersonalTimelineEvent(
        id: "older",
        friendshipID: selected,
        kind: .visitArrived,
        occurredAt: Date(timeIntervalSince1970: 1)
    )
    let newer = PersonalTimelineEvent(
        id: "newer",
        friendshipID: selected,
        kind: .visitReturned,
    ...
    let friend = FriendProfile(
        friendshipID: selected,
        accountID: AccountID(rawValue: "account-friend"),
        accountName: "阿乐",
        petID: PetProfileID(rawValue: "pet-friend"),
        petName: "团子",
        friendsSince: Date(timeIntervalSince1970: 1)
    )
    let model = SharedSpaceModel()
    model.friends = [friend]
    model.selectedFriendshipID = selected
    model.friendTimelineEvents[selected] = [older, newer]
    model.timelineEvents = [older, newer]
    model.friendCharacterIDs[friend.petID] = .malteseWhite

    #expect(model.selectedFriend?.petName == "团子")
    #expect(model.selectedFriendEvents.map(\.id) == ["newer", "older"])
    #expect(model.reversedTimelineEvents.map(\.id) == ["newer", "older"])
    #expect(model.characterID(for: friend) == .malteseWhite)
}
