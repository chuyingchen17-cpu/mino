...
func realScenePlaybackAdvancesAllRuntimePathsWithoutMovingAnchors() throws {
    ...
    for _ in 0..<2 {
        ...
        #expect(scene.hasActiveFrameMotionForTesting)
        harness.advance(by: petAnimation.frameDuration * 1.25)
        let advancedTexture = try #require(scene.currentTextureForTesting)
    ...
    #expect(
        scene.currentTextureForTesting
            === petAnimation.textures[petAnimation.frameIndex(progress: 0.63)]
    )
    #expect(!scene.hasActiveFrameMotionForTesting)
...
@MainActor
@Test
func longWalkReactionKeepsAuthoredFrameRateInsteadOfStretching() throws {
    let scene = PetScene(size: CGSize(width: 170, height: 180))
    let harness = try FrameSceneHarness(scene: scene)
    let animation = try #require(
        PetFrameAnimationCatalog.shared.animation(for: .malteseWhite, clip: .walk)
    )
    let plan = PetReactionPlan(
        speech: "走走",
        activity: .walking,
