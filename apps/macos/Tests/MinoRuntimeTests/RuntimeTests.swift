...
func localActivityReturnsToIdleAfterItFinishes() async throws {
    ...
    for _ in 0..<40 {
        ...
        if world.pets[.mine]?.activity == .idle {
            restored = true
            break
        }
    }
...
func kissInteractionApproachesThenEmitsEffectAndCompletes() {
    ...
    var interaction = PairedPetInteractionSession(
        choreography: .kissHeart,
        giver: mine,
        receiver: partner,
    ...
    for _ in 0..<200 where cue == nil {
        cue = interaction.advance(giver: &mine, receiver: &partner, deltaTime: 1.0 / 30.0).cue
    }
    ...
    let completion = interaction.advance(giver: &mine, receiver: &partner, deltaTime: 2)
...
func flowerInteractionHasDistinctOfferPoseAndCompletionEmotion() {
    ...
    let completion = interaction.advance(giver: &mine, receiver: &partner, deltaTime: 3)
    #expect(completion.completed)
    #expect(mine.emotion == .content)
    #expect(partner.emotion == .happy)
}
...
@Test
func pairedChoreographyCanDescribeANewPlayTogetherWithoutANewSessionType() {
    let playTogether = PairedPetChoreography(
        id: "play_together",
        cue: .none,
        ...
        receiverSpeed: 180,
        poseDuration: 1.2,
        giverPose: PairedPetPose(activity: .playing, emotion: .playful, clip: .play),
        receiverPose: PairedPetPose(activity: .playing, emotion: .happy, clip: .play),
        giverRestEmotion: .playful,
    ...
    var posed = false
    for _ in 0..<200 where !posed {
        let advance = interaction.advance(giver: &mine, receiver: &partner, deltaTime: 1.0 / 30.0)
        posed = interaction.phase == .posing
        #expect(advance.cue == nil)
    }
    #expect(posed)
    #expect(mine.activity == .playing)
    #expect(partner.activity == .playing)
    #expect(mine.motionClipOverride == .play)
    #expect(partner.motionClipOverride == .play)

    let completion = interaction.advance(giver: &mine, receiver: &partner, deltaTime: 2)
    #expect(completion.completed)
    #expect(completion.cue == nil)
    #expect(mine.emotion == .playful)
    #expect(partner.emotion == .happy)
}
